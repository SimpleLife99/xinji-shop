    ...
    <div class="cart">
      ...
      <div v-else>
        ...
        <div class="cart-play">
            ...
            <div class="play-right" @click="$router.push({ name: 'order' })">
                下单买({{cartList.length}})
            </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    // 减商品数量的操作
    reduce(index){
       this.cartList[index].cart_nums <=1 ? "" : this.cartList[index].cart_nums--
    },  
    // 加商品数量操作  
    plus(index){
        this.cartList[index].cart_nums >= this.cartList[index].commodity_stock ? "" : this.cartList[index].cart_nums++
    },
    /**
     * 获取购物车列表的数据
     */
    async getCartList(){
      let shoplist  = await this.$http.post({ url: "/cart/selectCart", token: this.$utils.encrypt({  userid: this.$utils.getUserInfo().id  }) })
  ...
  computed: {
      totalPrice(){
        ...
        for (const key in this.cartList) {
           total += this.cartList[key].cart_nums * this.cartList[key].commodity_nowprice
        }
        return total
      }
  },
